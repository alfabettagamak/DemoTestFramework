...
namespace Selenium
{
    public class TestBase
    {
        ...
        {
            JObject config = new Configuration().GetConfig();
            baseUrl = $"{config["site"]?["protocol"]}{config["site"]?["host"]}:{config["site"]?["port"]}/";
            ...
            var path = $"{Environment.CurrentDirectory}/testData.json";
        ...
        protected void Authorization(string login,  string password)
        {
            ...
            driver.Navigate().GoToUrl(baseUrl + "cabinet/");
        }
    }
}